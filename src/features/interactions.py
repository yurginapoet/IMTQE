...
def interaction_features(feats: Mapping[str, float]) -> dict[str, float]:
    ...
    Отсутствующие базовые ключи трактуются как 0.0 (кроме length_ratio=1.0, perplexity>=0).
    """
    ...
    emb_d = float(feats.get("embedding_distance", 0.0))
...
def add_interaction_columns_to_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    emb_d = _series(df, "embedding_distance", 0.0)
    ...
    df["dist_x_logppl"] = emb_d * log_ppl
    return df
